"""
RFF bandwidth/alpha sweep on Boxing learned representation.
Run in tmux: python rff_sweep_boxing.py
"""
...
lam = 0.9
gamma = 0.99
num_episodes = 300  # shorter for sweep
eval_last = 100

bandwidths = [2.0, 4.0, 8.0, 12.0, 16.0, 24.0]
...
for bandwidth in bandwidths:
    for alpha in alphas:
        ...
        print(f"[{config_num:2d}/{total_configs}] BW={bandwidth:5.1f} | "
              ...
              f"Steps: {total_steps:>8d} | Time: {elapsed:.0f}s")

# ---- Summary ---- #
print("\n" + "=" * 80)
print("RESULTS SUMMARY")
print("=" * 80)
print(f"{'BW':>6s} | {'Alpha':>6s} | {'Last ' + str(eval_last):>10s} | {'Overall':>10s} | {'Steps':>10s} | {'Time':>6s}")
print("-" * 65)

for (bw, al), res in sorted(results.items()):
    print(f"{bw:6.1f} | {al:6.3f} | {res['avg_last']:10.1f} | {res['avg_all']:10.1f} | {res['total_steps']:10d} | {res['time']:5.0f}s")
...
save_path = f"rff_sweep_boxing_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
